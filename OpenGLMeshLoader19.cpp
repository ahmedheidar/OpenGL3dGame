...
void myDisplay(void)
{
	...
	glPushMatrix();
	glScaled(0.05, 0.03, 0.05);
	glTranslated(400, 20, 450);
	model_rock.Draw();
	glPopMatrix();
	glPushMatrix();
	glScaled(0.05, 0.03, 0.05);
	glTranslated(-200, 30, 500);
	model_rock.Draw();
	glPopMatrix();
	glPushMatrix();
	glScaled(0.05, 0.03, 0.05);
	glTranslated(-300, 20, 100);
	model_rock.Draw();
	glPopMatrix();
...
void playerRotationController() {
	if (playerRotationControl % 4 == 0) {
		playerZ = originalPlayerZ;
...
void SpecialKey(int key, int x, int y) {
	...
	case GLUT_KEY_UP:
		if (playerZ < 400) {
			playerZ += 1;
			//model_player.pos.z = playerZ;
		}
		rotateTheplayer = false;
		break;
	case GLUT_KEY_DOWN:
		if (playerZ > -400) {
			playerZ -= 1;
